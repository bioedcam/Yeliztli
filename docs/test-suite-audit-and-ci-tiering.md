...
> - **Part 2 CI tiering — implemented (`#325`).** Path filters (`dorny/paths-filter`), Linux-only PR matrices, the `ci-required` aggregator, the nightly cross-OS backstop, and the `slow`-marks on `test_cross_module_integration` + `test_e2e_pipeline` are all present and verified. **Branch protection** requiring `CI Required` + `Lint` is now active (ruleset "main protection", 2026-06-09). The proposed **merge queue is N/A** — it requires an organization-owned repo and `bioedcam/Yeliztli` is a personal account, so the Tier-2 macOS/Docker/E2E gating stays **post-merge (`push`) + nightly** rather than pre-merge (the lines below proposing required *merge-queue* checks are superseded by this constraint — see the remediation plan's "Owner — repo settings").
...
> - **The hom_ref negative-control convention has propagated to new code.** Every expansion-wave risk-genotype module (thrombophilia, alpha-1, AMD, APOL1, gout, HFE, MT-RNR1, LHON, Parkinson's, sex-aneuploidy) ships an all-reference→no-finding negative control. The last **ClinVar-carriage finder** still missing one is `carrier_status` (the panel-scoring modules, `pharmacogenomics` and `nutrigenomics`, are gated on diplotype/genotype rather than ClinVar carriage, so they use separate wildtype→no-finding cases — tracked in the remediation plan).
>
